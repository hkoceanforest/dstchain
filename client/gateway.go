...
func (this GatewayClient) QueryModuleParameters(module string) (interface{}, error) {
	...
	if module == "gov" {
		...
		reponseStr, err := GetRequest(chainnet.Current.GetApiServerUrl(), "/"+module+"/parameters")
		if err != nil {
...
func (this *GatewayClient) QueryGatewayClusters(gatewayAddress string) (resp map[string]map[string]daotypes.ClusterDeviceMember, err error) {
	...
	if err != nil {
		if err.Error() != core.EmptyGetClustertoGate.Error() {
...
func (this *GatewayClient) MultiMsgBroadcast(multiMsg []sdk.Msg, privateKey string, fee legacytx.StdFee) (tx ttypes.Tx, resp *core.BroadcastTxResponse, err error) {
	...
	tx, result, err = this.TxClient.SignAndSendMsg(from.Address, privateKey, fee, txBase.Memo, multiMsg...)
	if err != nil {
		return
	}
	resp = new(core.BroadcastTxResponse)
