...
func (c *baseRPCClient) PoolFirstAndLastTxs(ctx context.Context) (*ctypes.PoolFirstAndLastTxs, error) {
	result := new(ctypes.PoolFirstAndLastTxs)
	_, err := c.caller.Call(ctx, "pool_firstlast_txs", map[string]interface{}{}, result)
	if err != nil {
		return nil, err
	}
	return result, nil
...
func (c *baseRPCClient) TxExisted(ctx context.Context, tx types.Tx) (*ctypes.ResultTxExisted, error) {
	result := new(ctypes.ResultTxExisted)
	_, err := c.caller.Call(ctx, "pool_tx_existed", map[string]interface{}{"tx": tx}, result)
	if err != nil {
		return nil, err
	}
	return result, nil
