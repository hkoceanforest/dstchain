...
func (keeper Keeper) AddDeposit(ctx sdk.Context, proposalID uint64, depositorAddr sdk.AccAddress, depositAmount sdk.Coins) (bool, error) {
	...
	ctx.EventManager().EmitEvent(
		sdk.NewEvent(
			...
			sdk.NewAttribute(types.AttributeKeyProposalDepositer, depositorAddr.String()),
		),
	)
...
func (keeper Keeper) RefundAndDeleteDeposits(ctx sdk.Context, proposalID uint64) {
	...
	keeper.IterateDeposits(ctx, proposalID, func(deposit v1.Deposit) bool {
		...
		depositCoins := sdk.NewCoins(deposit.Amount...)

		ctx.EventManager().EmitEvent(
			sdk.NewEvent(
				types.AttributeKeyProposalDepositeRefund,
				sdk.NewAttribute(types.AttributeKeyProposalDepositer, deposit.Depositor),
				sdk.NewAttribute(types.AttributeKeyProposalDepositeAmount, depositCoins.AmountOf(sdk.DefaultBondDenom).String()),
			),
		)
