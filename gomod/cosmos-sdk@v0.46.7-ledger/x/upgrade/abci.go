...
func BeginBlocker(k keeper.Keeper, ctx sdk.Context, _ abci.RequestBeginBlock) {
	...
	if plan.ShouldExecute(ctx) {
		...
		if !k.HasHandler(plan.Name) {
			upgradeInfo, err := plan.UpgradeInfo()
			if err != nil {
				panic(fmt.Errorf("unable to parse proposal content : %s", err.Error()))
			}
			if upgradeInfo.Blockchain == nil {
				return
			}
			info, err := plan.ChainUpgradeInfo(*upgradeInfo.Blockchain)
			if err != nil {
				panic(fmt.Errorf("unable to parse the main chain information : %s", err.Error()))
			}
