...
func NewKeeper(storeKey storetypes.StoreKey, cdc codec.Codec, router *baseapp.MsgServiceRouter, accKeeper group.AccountKeeper, bankKeeper bankkeeper.Keeper, config group.Config, ps paramtypes.Subspace) Keeper {
	k := Keeper{
		key:        storeKey,
		router:     router,
		accKeeper:  accKeeper,
	...
	if !ps.HasKeyTable() {
		ps = ps.WithKeyTable(group.ParamKeyTable())
	}
	return k
}
...
func (k Keeper) TallyProposalsAtVPEnd(ctx sdk.Context) error {
	...
	for _, proposal := range proposals {
		...
		if proposal.Status == group.PROPOSAL_STATUS_ACCEPTED && proposal.ExecutorResult != group.PROPOSAL_EXECUTOR_RESULT_SUCCESS {
			...
			if err != nil {
				return sdkerrors.Wrap(err, "Exec")
			}
			...
			if results, err := k.doExecuteMsgs(cacheCtx, k.router, proposal, addr, decisionPolicy); err != nil {
				...
				amounts := k.GetParams(ctx).Deposit
				fromAddress, _ := sdk.AccAddressFromBech32(proposal.Proposers[0])
				err = k.bankKeeper.SendCoinsFromModuleToAccount(ctx, group.ModuleName, fromAddress, amounts)
				if err != nil {
					return sdkerrors.Wrap(err, "group SendCoinsFromAccountToModule error")
				}
			}
		}
		//if proposal.ExecutorResult == group.PROPOSAL_EXECUTOR_RESULT_SUCCESS {
		//	// if err := k.pruneProposal(ctx, proposal.Id); err != nil {
		//	// 	return sdkerrors.Wrap(err, "pruneProposal")
		//	// }
		//} else {
		//	store := ctx.KVStore(k.key)
		//	if err := k.proposalTable.Update(store, proposal.Id, &proposal); err != nil {
		//		return sdkerrors.Wrap(err, "proposalTable Update")
		//	}
		//}
