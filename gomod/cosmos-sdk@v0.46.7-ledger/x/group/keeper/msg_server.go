...
func (k Keeper) SubmitProposal(goCtx context.Context, req *group.MsgSubmitProposal) (*group.MsgSubmitProposalResponse, error) {
	...
	amount := sdk.NewCoins(k.GetParams(ctx).Deposit...).AmountOf("dst")
	fromAddress, _ := sdk.AccAddressFromBech32(proposers[0])
	err = k.bankKeeper.SendCoinsFromAccountToModule(ctx, fromAddress, group.ModuleName, sdk.NewCoins(sdk.NewCoin("dst", amount)))
	if err != nil {
		return nil, sdkerrors.Wrap(err, "group SendCoinsFromAccountToModule error")
	}
	...
	ctx.EventManager().EmitEvent(
		sdk.NewEvent(
			group.EventTypeGroupDeposit,

			sdk.NewAttribute(group.AttributeGroupProposer, req.Proposers[0]),
			sdk.NewAttribute(group.AttributeGroupAmount, amount.String()),
			sdk.NewAttribute(group.AttributeGroupModuleAddress, authtypes.NewModuleAddress(group.ModuleName).String()),
		),
	)
...
func (k Keeper) doTallyAndUpdate(ctx sdk.Context, p *group.Proposal, electorate group.GroupInfo, policyInfo group.GroupPolicyInfo) error {
	...
	if !ctx.BlockTime().After(p.VotingPeriodEnd) {
		return sdkerrors.Wrap(sdkerrors.New("10", 1133, "Voting is not end"), "Voting is not end")
	}
...
func (k Keeper) GetGroupInfo(goCtx context.Context, groupID uint64) (group.GroupInfo, error) {
	ctx := sdk.UnwrapSDKContext(goCtx)
	var obj group.GroupInfo
	_, err := k.groupTable.GetOne(ctx.KVStore(k.key), groupID, &obj)
	return obj, err
}
func (k Keeper) GetGroupMember(ctx sdk.Context, member *group.GroupMember) (*group.GroupMember, error) {
	var groupMember group.GroupMember
